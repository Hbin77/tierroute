...
types, collection/text bounds, and outer/nested LODO amplification checks. Do not add an
environment or CLI bypass. If official data exceeds the contract, record authenticated
shape measurements and use a reviewed constant change or a separate schema adapter
with exact-limit and limit-plus-one tests.
