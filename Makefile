...
.PHONY: help install install-dev check-install lint test licenses spdx smoke verify reproduce
...
help:
	...
	@echo "  install-dev          install exact dev lock and editable tierroute"
...
verify: lint spdx test licenses check-install smoke
