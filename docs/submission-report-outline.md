...
| `I-GBM-934AAB6` | Dependency-free per-model squared-error regression-stump boosting uses deterministic split/tie rules, pre-embedding work guards, inner-LODO OOF predictions, and per-model isotonic calibration | [`predictors/gbm.py`](../src/tierroute/predictors/gbm.py), [`predictors/gbm_training.py`](../src/tierroute/predictors/gbm_training.py) at `934aab6` | [`test_gbm_core.py`](../tests/test_gbm_core.py), [`test_gbm_training.py`](../tests/test_gbm_training.py), [PR #41 CI run `29489429927`](https://github.com/Hbin77/tierroute/actions/runs/29489429927) | This proves deterministic, leakage-controlled library wiring only; no artifact, CLI, matched comparison, or predictive-gain evidence |
...
| In-memory deterministic stump-GBM core with per-model isotonic calibration | `IMPLEMENTED — LIBRARY ONLY` | `I-GBM-934AAB6` |
