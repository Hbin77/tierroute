...
reference는 canonical row와 domain별 Welford/Chan 통계·subset 격리를 구현했지만
provider·solve·score 경로는 아직 없다는 경계를 구분한다. binary64 feature-cache
추정치를 binary32로 조용히 바꾸면 calibrator, lambda, 최종 report까지 parity가
필요한 이유를 설명한다. 전용 근거는
