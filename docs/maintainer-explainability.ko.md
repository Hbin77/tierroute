...
별도의 prepared execution 경로는 production trainer나 native·persistent session이
아닌 bounded in-memory Python reference다. active raw coordinate를 `a_i`, continuous
...
계획한 RouterBench 전체 shape는 이전 store/statistics/reference cap에서 거부된다.
Bounded 4~7-domain fixture의 calibration·lambda·최종 report 배선 근거는 이제
있지만 provider·persistence·native execution·all-domain artifact·scalable
official-shape parity·성능·공식 데이터 근거는 없으며 issue #9는 열려 있다.
...
[네이티브 protocol](native-ridge-protocol.md)에 있다.
...
| feature, ridge/GBM 예측기, prepared reference execution, paired 추정, calibration |  |  |  |  |
