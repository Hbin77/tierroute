...
수치는 이후 prepared policy bridge를 포함하지 않는다. 제출 전에 bridge 자체 branch
구현 `63e288e`, test `3249a3c`와 Darwin arm64 로컬 Python 3.10 954 passed,
Python 3.12 953 passed·expected compatibility skip 1을 별도로 기록했다. Branch CI는
아직 별도로 기록해야 한다. 자동 CI는 아래 owner walkthrough를 대신하지 않는다.
