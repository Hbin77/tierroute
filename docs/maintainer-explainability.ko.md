...
Python 증명 코드로 남아 있고, 새 native·persistent session과 통합되지 않았다.
active raw coordinate를 `a_i`, continuous
scale을 `s_i`(비연속 coordinate는 1)로 두면
...
이 adapter는 CLI·policy·calibration·lambda·all-domain artifact·prepared 6-baseline과
통합되지 않았다.
...
embedding을 투영하지 않은 `D4/N8/d1036/M1` 완료 근거는 있다. 하지만
official `D7/N34778/d1036/M11` 전체 store/session, provider, policy·all-domain artifact,
CLI, prepared 6-baseline, Linux-musl·3플랫폼 배포 artifact, 성능·품질·비용, 공식 데이터 근거는
없으며 issue #9는 열려 있다.
...
| feature, ridge/GBM 예측기, prepared reference·file/native session, paired 추정, calibration |  |  |  |  |
