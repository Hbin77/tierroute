...
접히는지 유도하고, 현재 계약이 이를 열거·preflight할 뿐 실행하지 않는 경계도
구분한다. binary64 feature-cache 추정치를 binary32로 조용히 바꾸면 calibrator,
lambda, 최종 report까지 parity가 필요한 이유를 설명한다. 전용 근거는
