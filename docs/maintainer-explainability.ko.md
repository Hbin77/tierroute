...
provider·persistence·native execution·calibration·lambda·최종 report·성능·공식 데이터
근거는 없으며 issue #9는 열려 있다. binary64 feature-cache를 binary32로
...
python -m pytest -q tests/test_prepared_graph.py tests/test_prepared_store.py tests/test_prepared_execution.py
git apply <<'PATCH'
