...
CLI, prepared 6-baseline, 3플랫폼, 성능·품질·비용, 공식 데이터 근거는
없으며 issue #9는 열려 있다.
...
case 35개를 포함해 58 passed를 기록했다. 작은 surface-only fixture의 실제 compiled
...
RSS·memory efficiency·throughput·품질·비용 결과가 아니다. 새 prepared source의
macOS/Windows CI와 사람 서명도 pending이다.
...
hardening은 코드 근거이지 아직 platform CI 근거가 아니라는 경계도 분리한다.
