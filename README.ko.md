...
prepared moment solve/raw-score 참조 경로·실험적 인증 file-backed prepared C11
solve/score session과 bounded 쿼리별 policy-benchmark bridge·외부 데이터가 필요 없는
데모가 구현되어 있습니다.
...
In-memory reference들은 provider 추론과 file I/O를 하지 않습니다. Native slice는 이제
위 bounded public Python API를 통해 인증된 file-backed score를 calibration, lambda 평가,
learned report, 베이스라인 6종에 연결합니다. 이 연결은 쿼리별 accounting과 프로젝트 작성
fixture에 고정되며 all-domain 배포 artifact를 만들거나 제공 명령·trainer를 바꾸지 않습니다.
Tolerance-close raw score가 새 데이터의 모든 PAV partition과 exact 결정을 보존한다고
보장하지 않으므로 이후 데이터별 parity는 epsilon을 추가하지 않고 직접 비교해 불일치 시
실패해야 합니다. 숫자 digest도 cross-platform 보장이 아닌 local 근거입니다. 이 bridge에서
bge-m3, 공식 데이터, RouterBench, 품질, 비용 절감, 성능 결과가 따라오지 않습니다. 따라서
...
official-shape 실행, 3플랫폼 release-artifact 검사가 끝날 때까지 gate로 남습니다. Native
