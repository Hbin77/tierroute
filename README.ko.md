...
빠른 경로는 `tierroute train`과 bilinear/lambda-policy 학습 smoke를 건너뛰고 설치된
합성 predictor, artifact 로드, 라우팅, 평가, 데모를 실행합니다. 평가와 데모는 필수
outer-LODO domain-table baseline을 여전히 맞춥니다. 전체 경로는 lint, SPDX, test,
license, 설치 검사에 이어 합성 predictor와 policy artifact를 학습하고 다시 소비합니다.
