...
빠른 경로는 predictor나 policy를 학습하지 않고 설치된 합성 predictor, artifact 로드,
라우팅, 평가, 데모를 실행합니다. 전체 경로는 lint, SPDX, test, license, 설치 검사에 이어
합성 predictor와 policy artifact를 학습하고 다시 소비합니다. `make reproduce`는 전체
경로의 호환 alias입니다. 이 설치 target들은 환경을 검토된 exact 개발 lock으로
교체하므로 반드시 전용 가상환경 안에서 실행합니다.
