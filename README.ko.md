...
bilinear 학습·별도의 canonical 라이브러리 수준 v1 artifact를 갖는 결정론적
stump-GBM 학습기·두 계열의 paired 기술 추정·exact tier-lambda 튜닝·엄격한
bilinear 예측기 v1/정책 artifact·정확한 견적-실현 비용 오차 지표·bounded
prepared moment solve/raw-score 참조 경로·offline canonical prepared all-domain
predictor artifact·실험적 인증 file-backed prepared C11 solve/score session과 bounded
쿼리별 policy-benchmark bridge·외부 데이터가 필요 없는 데모가 구현되어 있습니다.
...
특징 bilinear artifact는 엄격한 canonical JSON이며 생성에 쓴 solver ID도 기록합니다.

아래 배포 명령은 계속 bilinear 전용입니다. GBM에는 feature schema, 결정론적 stump
ensemble, 모델별 isotonic calibrator, 학습 설정과 학습 범위 identity를 보존하는
별도의 canonical JSON artifact v1이 구현되어 있습니다. 이 artifact는 Python
라이브러리에서 save/load하고 predictor를 복원할 수 있지만 `train`/`route`/showcase
및 policy artifact와는 연동되지 않습니다. `compare-predictors`도 계속 두 고정 계열을
선택 없이 기술적으로 비교할 뿐입니다. 이 영속화 계약은 기존 bilinear predictor
artifact v1 bytes를 바꾸지 않으며 외부·공식 데이터, bge-m3, 배포, 성능, 품질
향상 또는 비용 절감 근거를 제공하지 않습니다.
...
별도의 bounded Python 라이브러리 reference인
[prepared all-domain predictor artifact](docs/prepared-all-domain-artifact.md)는 호출자가
...
별도의 실험적
[native prepared-session protocol](docs/native-prepared-session-protocol.md)은 기본 trainer와
CLI를 바꾸지 않고 인증된 file-backed vertical slice를 추가합니다. 고정
little-endian `TRPSTO01` container는 canonical row key, domain index, binary64
feature와 target을 저장합니다. 호출자가 고정한 receipt는 whole file, source-fit
identity, logical prepared-store identity, 선택적 사전 계산 embedding snapshot을
묶습니다. `TRPSES01` adapter는 이 store와 사용자가 명시한 로컬 실행 파일을
각각 인증하고, descriptor가 안정적인 private copy를 만든 뒤 전체 graph를
preflight하고 C11 child를 정확히 한 번 실행합니다. 성공한 `TRPRES01` 응답은
모든 coefficient/raw-score block을 담고 context가 닫힐 때까지 mmap-backed
상태를 유지합니다. 이 hash들은 신뢰하는 expected 값과 비교해 정확한 byte만
인증할 뿐 provenance, compiler, 네트워크 동작을 승인하지 않습니다.

Public Python entry point
[`evaluate_native_prepared_per_query_benchmark`](src/tierroute/policies/native_prepared_benchmark.py)는
호출자가 소유하고 이미 인증한 열린 `NativePreparedSessionResult`를 소비합니다. 호출자는
별도로 보관한 expected binary·result-file SHA-256과 신뢰한 prepared-store receipt도
제공해야 하며, receipt의 whole-file digest가 외부 store pin입니다. 1단계는 deep evaluation
순회 전에 이 credential을 비교하고 현재 result mapping을 다시 hash한 뒤, owner-only
prepared-store snapshot을 인증하고 native 숫자 view를 `at()`으로 cell별 읽습니다. 호출자
코드를 부르지 않고 bounded owned calibrated snapshot을 만들고, 마지막 mapped read 뒤 두
입력을 다시 인증하고, 자신이 소유한 store snapshot을 닫은 다음 계속 열려 있는 caller-owned
result를 더는 읽지 않습니다. 2단계는 owned snapshot과 고정 `PerQueryBudgetLedger`만으로
nested-LODO lambda 평가와 베이스라인 6종을 실행합니다. 이 API에는 호출자 ledger,
cumulative accounting, cascade control이 들어오지 않습니다. Durable result는 calibration
parameter와 content identity를 남기지만 mmap, native view, raw-score matrix, target matrix,
calibrated-score payload를 반환 graph에 남기지 않습니다.

프로젝트 작성 소스는 의존성이 없고 sdist에만 포함되며 prepared binary나 native
source는 wheel에 들어가지 않습니다. source checkout의 helper는 명시한 절대
compiler 경로만 쓰고 `PATH`를 탐색하거나 다운로드하지 않습니다.
...
이는 Python API/test 경로이며 어떤 `tierroute` 명령에도 연결되지 않았습니다.
...
현재 high-level native-policy 테스트는 surface-only D4, D5, D6, D7을 다룹니다. Fold,
선택 call, budget, evidence identity를 포함한 learned result 전체와 동일한 베이스라인 6종
result가 authoritative rowwise 경로와 엄격히 같아야 합니다. D7 fixture는 domain 행 수
`(1, 2, 1, 3, 2, 1, 2)`와 모델 3개를 사용하는 의도적인 불균등 사례입니다. 적대적
테스트는 잘못된 외부 binary/result/store credential이 store 인증 전에 실패하는지,
deep evaluation 순회 전에 result integrity를 확인하는지, 마지막 mapped read 후 외부
result pin을 다시 대조하는지, 지속 mutation·bit-exact target 불일치가 거부되는지,
native view를 `at()`으로만 읽는지, learned/baseline replay 전에 owned store가 닫히는지,
cleanup도 실패할 때 primary error가 보존되는지, 반환 graph에 mapped view나 score payload가
없는지를 고정합니다. Focused native 실행은 89 passed입니다. 잠긴 전체 suite에서 Python
...
wall-time 보장이 아닙니다. 고정한 RouterBench/bge-m3 전체 크기는 bounded
in-memory feature-store, statistics, reference-execution cap에 의해 계속 거부됩니다.
별도의 file/native preflight가 그 aggregate shape를 허용하지만, 이는 materialized
execution, 수치 parity, benchmark 근거가 아닙니다.

Graph/store/execution/policy in-memory reference들은 provider 추론과 file I/O를 하지
않습니다. 별도의 all-domain artifact는 명시적인 canonical local persistence만 추가하며,
assembly와 persistence 중 provider 추론은 여전히 없습니다. Native slice는 이제 위 bounded
public Python API를 통해 인증된 file-backed score를 calibration, lambda 평가, learned
report, 베이스라인 6종에 연결합니다. 이 연결은 쿼리별 accounting과 프로젝트 작성 fixture에
고정되며 native result를 별도의 bounded all-domain artifact에 공급하거나 제공 명령·trainer를
바꾸지 않습니다. Tolerance-close raw score가 새 데이터의 모든 PAV partition과 exact 결정을
보존한다고 보장하지 않으므로 이후 데이터별 parity는 epsilon을 추가하지 않고 직접 비교해
불일치 시 실패해야 합니다. 숫자 digest도 cross-platform 보장이 아닌 local 근거입니다. 이
bridge에서 bge-m3, 공식 데이터, RouterBench, 품질, 비용 절감, 성능 결과가 따라오지
않습니다. 따라서
[Issue #9](https://github.com/Hbin77/tierroute/issues/9)는 아직
열려 있습니다.

계획된 1,024차원 bge-m3 임베딩과 표면 특징을 합친 최대 1,036차원 전체 학습은 감사된
offline local provider, authenticated prepared session을 bounded all-domain artifact가
소비하는 경로, CLI 재현,
...
- 모델별 regression stump ensemble, 고정된 순서·동률 규칙, 임베딩 호출 전 보수적
  ...
  별도로 versioning한 canonical GBM predictor artifact v1이 bounded strict JSON
  라이브러리 API로 이 상태를 보존합니다. CLI나 policy artifact는 이를 사용하지
  않으며 기존 bilinear predictor artifact v1 canonical bytes는 바뀌지 않습니다.
...
- canonical 정책 artifact는 정확한 predictor hash, 학습/지표에 관련된 replay
  ...
  포함합니다.
- 기존 bilinear 예측기와 정책 파일은 배타적인 무작위 staging, 저장 후 검증, 정책을 마지막에 교체하는
...
- Bounded prepared all-domain predictor artifact 라이브러리 API는 caller-pinned 6단계
  resnapshot·association·final-solve·calibration assembly를 수행하고 exact content
  lineage를 가진 strict canonical JSON을 저장합니다. Assembly, save, pinned load는
  provider-free·offline입니다. 합성 D4-D7, high-dynamic-range, 독립 PAV, one-shot route
  검증은 wiring·parity 근거이지 unbiased quality 근거가 아닙니다. 자세한 경계는
  [artifact 계약](docs/prepared-all-domain-artifact.md)에 있습니다.
- Bounded prepared policy bridge는 `C(D,2)+D` inner-LODO calibration context와 `D^2`
  ...
  noninterference를 테스트합니다. 이 bridge는 별도의 predictor artifact를 생성하거나
  소비하지 않으며 보편적 exact-parity를 주장하지 않습니다.
...
- Public 실험 Python bridge는 caller-owned open native result와 필수 외부
  binary/result/store pin을 2단계로 소비합니다. 자신이 소유한 store snapshot을 닫은 뒤
  고정 쿼리별 nested-LODO learned replay와 베이스라인 6종을 실행하고 payload-free evidence를
  반환합니다. 불균등 3-model D7을 포함한 surface-only D4-D7 fixture는 전체 rowwise
  benchmark result와 엄격히 같고, credential·mutation·접근 경로·cleanup·return graph
  테스트가 fail closed를 고정합니다.
...
사용합니다. 제공하는 CLI 중 GBM을 학습하는 경로는 계속 비배포
paired-estimation runner뿐입니다. Canonical GBM artifact v1은 Python 라이브러리에서만
사용할 수 있으며 CLI·policy·배포 연동, 로컬 bge-m3 backend와 asset, 라이선스가
확인된 보고 가능 계열 선택 실험은 아직 계획 항목입니다. 외부·공식 데이터,
새 의존성·SBOM inventory, 성능·품질 향상·비용 절감 근거를 추가하지 않았고 기존
bilinear predictor artifact v1 bytes도 바뀌지 않았습니다. 어떤 예측기 계열의 우위도
주장하지 않습니다.
...
predictors/  bilinear·GBM 학습/canonical artifact, 모델별 calibration
...
최대 1,036개 전체 특징 학습은 위 provider, all-domain artifact와
native/provider/CLI/policy 경로의 통합, official-shape 실행, 3플랫폼 release-artifact
검사가 끝날 때까지 gate로 남습니다. Native
prepared slice는 작은 D4-D7 surface-only solve/score parity를, bounded policy consumer는
불균등 3-model D7을 포함한 D4-D7 rowwise learned+6-baseline strict parity를 입증합니다.
별도의 1,036-feature 합성 session은 투영 없이 완료됐지만 공식 전체 tuple은
preflight-only입니다. 어느 것도 전체 nested 실험을 실행했다는 근거가 아니며, 임베딩
차원을 조용히 투영해 버리지 않습니다.
