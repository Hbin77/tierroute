...
bilinear 학습·인메모리 결정론적 GBM 참조 학습기·exact tier-lambda 튜닝·엄격한
bilinear 예측기 v1/정책 artifact·정확한 견적-실현 비용 오차 지표·외부 데이터가
필요 없는 데모가 구현되어 있습니다.
...
단일 라우팅, 베이스라인 6종 평가, 학습 라우터와 베이스라인 비교 벤치마크, 학습
기반 3단계 showcase를 차례로 실행합니다.
...
`demo`, `train`에는 `--json`을 쓸 수 있고, 버전이 명시된 호환 replay JSON을
평가와 벤치마크 입력으로 지정할 수 있습니다.
...
아래 명령은 계속 bilinear 전용입니다. 1단계 GBM API는 인메모리 전용이며 versioned
artifact, train/route/benchmark CLI 연동, 동등 범위 비교, 성능 결과를 제공하지
않습니다.
...
- 모델별 regression stump ensemble, 고정된 순서·동률 규칙, 임베딩 호출 전 보수적
  ...
  의존성 없는 squared-error gradient boosting. 합성 테스트는 알고리즘 배선만 입증
...
사용합니다. 결정론적 GBM 코어는 라이브러리 전용입니다. 로컬 `bge-m3` 임베딩
백엔드, GBM artifact/CLI 연동, 동등 범위 GBM/bilinear 비교 실험은 아직 계획
...
make reproduce-training PYTHON=python   # 전체 경로: 검사·학습·benchmark·demo
```

두 경로 모두 비어 있는 임시 Hugging Face cache를 만들고 offline mode를 강제합니다.
빠른 경로는 `tierroute train`, `tierroute benchmark`, `tierroute demo`, 모든
bilinear/lambda-policy fitting을 건너뛰고 설치된 합성 predictor, artifact 로드,
라우팅, 베이스라인 6종 평가를 실행합니다. 평가는 필수 outer-training domain
table만 맞추며 학습 predictor는 맞추지 않습니다. 전체 경로는 lint, SPDX, test,
license, 설치 검사에 이어 training smoke에서 합성 predictor/policy artifact를
학습·소비하고 nested-LODO benchmark와 학습 기반 3단계 demo를 실행합니다. 따라서
benchmark와 showcase fitting은 `training-smoke`와 `reproduce-training`에서만
실행되며 inference 경로에는 들어가지 않습니다.
