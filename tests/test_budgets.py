...
def test_decimal_boundary_is_charged_exactly(ledger_type: object) -> None:
    ...
    assert ledger.try_charge(Decimal("0.1")) is True
    assert ledger.try_charge(Decimal("0.2")) is True
...
def test_per_query_budget_resets_for_each_query() -> None:
    ...
    ledger.begin_query("q1")
    assert ledger.try_charge(Decimal("1")) is True
    ledger.finish_query()
...
def test_cumulative_budget_carries_remaining_amount() -> None:
    ...
    ledger.begin_query("q1")
    assert ledger.try_charge(Decimal("0.75")) is True
    ledger.finish_query()
    ledger.begin_query("q2")
    assert ledger.remaining_budget == Decimal("0.25")
    assert ledger.try_charge(Decimal("0.5")) is False
    ledger.finish_query()

    report = ledger.report()
    assert report.spent == Decimal("0.75")
